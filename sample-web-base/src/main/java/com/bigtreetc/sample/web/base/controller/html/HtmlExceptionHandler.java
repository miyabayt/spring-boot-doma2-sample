...
import com.bigtreetc.sample.domain.exception.FileNotFoundException;
...
public class HtmlExceptionHandler {
  ...
  public RedirectView handleOptimisticLockingFailureException(
    ...
    val view = getRedirectView(request, response, locale, ERROR_MESSAGE, messageCode);
  ...
  public RedirectView handleDoubleSubmitErrorException(
      Exception e, HttpServletRequest request, HttpServletResponse response) {
    if (log.isDebugEnabled()) {
  ...
  protected RedirectView getRedirectView(
    ...
    outputFlashMap(request, response, messageType, message);
   ...
   * @param value
   */
  protected void outputFlashMap(
      HttpServletRequest request, HttpServletResponse response, String attr, String value) {
